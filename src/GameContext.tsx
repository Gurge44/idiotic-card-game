    ...
    switch (action.type) {
        ...
        case "DRAW_CARD": {
            if (state.cards.length === 0)
                return { ...state, message: "No more cards left.... if you still want to suffer, reload the page, because I couldn't make the restart button work" };
        ...
        case "FIGHT_RESULT": {
            ...
            if (action.win) {
                for (const r of state.currentCard.reward) {
                    if (r === "❤") newHealth = Math.min(state.maxHealth, newHealth + 1);
                    if (r === "⚔") newDamage += 1;
                }
            } else {
                for (const p of state.currentCard.penalty) {
                    if (p === "❤") newHealth -= 1;
                    if (p === "⚔") newDamage = Math.max(0, newDamage - 1);
                }
            }

            return {...state, health: newHealth, damage: newDamage, message: action.message, currentCard: undefined, gameOver: newHealth <= 0,};
        }

        case "FLEE":
            if (!state.currentCard) return state;
            if (state.storedCards.length >= 3)
                return { ...state, message: "Bruh, how many do you want to store? That's unfair, you must lose more brain cells!" };
            return {
                ...state,
